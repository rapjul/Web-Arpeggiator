...
- Keeps the raw take for repeat exports while retaining decoded PCM only after a failed conversion; successful export, replacement, and teardown release it. See [ADR 0017](./docs/adr/0017-awaited-recording-lifecycle-and-bounded-audio-resource-ownership.md)
...
- Select **Include effects tail** to retain a conventional cold start and wait for the final scheduled gate to end. **Auto (recommended)** estimates the effective synth release and active delay, reverb, and chorus decay, capped at 10 seconds. `PluckSynth` uses its fixed one-second physical-model release rather than `envRelease`. **Custom** appends the chosen 0–10 second duration. Legacy settings without a tail strategy continue to use the previous explicit 2-second tail.
