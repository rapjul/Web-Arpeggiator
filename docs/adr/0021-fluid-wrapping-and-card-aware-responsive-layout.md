...
- **Flexbox Containment**: `#visualizer-viewport` specifies `min-width: 0;` and `#keyboard-main-wrapper` specifies `max-width: 100%; justify-content: safe center; overflow-x: auto; -webkit-overflow-scrolling: touch;` to guarantee flex parents never expand beyond viewport boundaries and that leftmost keys remain accessible via scrolling on ultra-compact displays.
- **Tooltip Containment and Discrete Transitions**: Custom tooltips (`.has-custom-tooltip`) declare `display: none; opacity: 0; visibility: hidden;` when inactive, ensuring 180px tooltips never expand ancestor `scrollWidth` on compact mobile viewports (e.g. 375px/390px). Modern browsers interpolate entry and exit animations smoothly via `@starting-style`, `transition-behavior: allow-discrete;`, and `--ui-transition-tooltip: 0.15s;`, while `@media (prefers-reduced-motion: reduce)` disables animations for users with motion sensitivity.
