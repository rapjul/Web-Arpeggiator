...
`RecorderManager` owns `idle`, `starting`, `recording`, `stopping`, and `destroyed` phases. It awaits `Tone.Recorder.start()` and native `MediaRecorder`'s `start` event before it publishes recording state or requests transport playback. It rejects competing transitions and ignores late backend events after destruction.

All start and stop paths observe synchronous exceptions, rejected promises, and native error events. If playback fails after capture starts, cleanup is attempted in a guarded path, the original playback error remains the rejected error, and UI recovery always runs. A valid blob from that cleanup remains exportable.

The raw recorded blob remains available until a replacement take or teardown. Decoded PCM is retained only when a WAV or MP3 conversion fails, allowing a retry without a redundant decode; a successful export, new blob, or teardown releases it. `destroy()` awaits active capture shutdown, disconnects graph targets, disposes Tone recorders, stops native stream tracks, clears handlers and chunks, and drops retained blobs and buffers. Runtime teardown awaits that operation before it disposes the audio engine.
