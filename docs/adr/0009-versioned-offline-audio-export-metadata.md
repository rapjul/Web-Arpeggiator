...
A rendered audio file alone cannot reveal the sound design, export mode, timing, or random pattern material used to make it. A settings snapshot is necessary for a future upload-and-restore workflow, but settings alone do not reproduce a random direction unless the materialized sequence is preserved as well.
...
The record includes the complete normalized settings serialization, the materialized scheduled note sequence and steps per loop, and render details: export mode, loop count, musical/pre-roll/tail/render durations, sample rate, channel count, and output frame count. Capturing the materialized sequence makes randomized exports inspectable and provides a future importer with a concrete comparison target.
